...
function printIndexAndValue() {
    const cars = ["BMW", "VW", "Audi"];
    cars.map((car, i) => console.log(i + ": " + car));
}

//3.
function printNames() {
    const friends = ["tadAs", "MaTaS", "lauRYnAs"];
    friends.map(friend => console.log(friend.charAt(0).toLocaleUpperCase() + friend.slice(1).toLowerCase()));
}